package com.android.kotlinmvvmtodolist.presentation.br
...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(p0: Context?, p1: Intent?) {
        ...
        if(sharedPreferences.getBoolean(taskInfo?.priority.toString(), true)){
            ...
            val action1 : NotificationCompat.Action = NotificationCompat.Action.Builder(0,"تمت",pendingIntent1).build()

            val notification = p0?.let {
                NotificationCompat.Builder(it, "to_do_list")
                    .setContentTitle("تذكير بالقضية")
                    .setContentText(taskInfo?.description)
                    .setSmallIcon(R.mipmap.lawyer)
                    .setAutoCancel(true)
